...
class StrategyEngine:
    def __init__(self):
        ...
        logging.info(f"Risk/Reward: {RISK_REWARD}, Volume Multiplier: {VOLUME_MULTIPLIER}")
    ...
    def detect_trade_signal(self) -> Optional[Dict]:
        ...
        try:
            ...
            # ✅ Volume logic: last 30 closed candle average vs last fully closed
            avg_volume = df['volume'].iloc[-32:-2].mean()
            ...
            if trend_up:
                logging.info(f"[DEBUG] Prediction: {prediction:.2f}, Volume OK: {volume_ok}, Trend: UP, Close > Prev High: {last_row['close'] > prev_row['high']}")
            elif trend_down:
                logging.info(f"[DEBUG] Prediction: {prediction:.2f}, Volume OK: {volume_ok}, Trend: DOWN, Close < Prev Low: {last_row['close'] < prev_row['low']}")
            else:
                logging.info(f"[DEBUG] Prediction: {prediction:.2f}, Volume OK: {volume_ok}, Trend: FLAT")

            if prediction > 0.55 and trend_up and last_row['close'] > prev_row['high'] and volume_ok:
            ...
            elif prediction < 0.45 and trend_down and last_row['close'] < prev_row['low'] and volume_ok:
                entry = last_row['close']
...
only change the probability if 65 up then send buy signal and if 35% low then sell and eress the volume and other nothing change 


and say whats the log output isee after update
