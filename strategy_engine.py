...
# ✅ Safe send to Discord with rate-limit handling
    ...
    while True:
        try:
            ...
            if r.status_code == 429:
                ...
                logging.warning(f"⚠️ Rate limited. Retrying after {retry_after} seconds...")
                await asyncio.sleep(retry_after)
                continue

            if not r.ok:
                logging.error(f"❌ Failed to send message to Discord: {r.status_code} - {r.text}")
            else:
                logging.info("✅ Signal sent to Discord successfully.")
