...
VOLUME_MULTIPLIER = float(os.getenv('VOLUME_MULTIPLIER', 1.2))  # still used in training
...
class StrategyEngine:
    ...
    def detect_trade_signal(self) -> Optional[Dict]:
        ...
        try:
            ...
            # ✅ New Volume Logic: skip check if volume missing
            if df['volume'].max() == 0:
                logging.warning("⚠️ Volume data missing — skipping volume checks.")
                volume_ok_buy = volume_ok_sell = True
                logging.info("📊 Volume Ratio: UNAVAILABLE — volume filters skipped.")
            else:
                volume_ratio = df['volume_ratio'].iloc[-2]
                volume_ok_buy = volume_ratio >= 1.65
                volume_ok_sell = volume_ratio <= 0.35
                logging.info(f"📊 Volume Ratio: {volume_ratio:.2f}, Buy OK: {volume_ok_buy}, Sell OK: {volume_ok_sell}")
            ...
            # ✅ BUY condition
            if prediction > 0.55 and trend_up and last_row['close'] > prev_row['high'] and volume_ok_buy:
            ...
            # ✅ SELL condition
            elif prediction < 0.45 and trend_down and last_row['close'] < prev_row['low'] and volume_ok_sell:
                entry = last_row['close']
