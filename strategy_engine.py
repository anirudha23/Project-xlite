...
class StrategyEngine:
    ...
    def detect_trade_signal(self) -> Optional[Dict]:
        ...
        try:
            ...
            # ✅ Use volume from indicator-aligned df
            avg_volume = df['volume'].iloc[-200:].mean()
            last_volume = df['volume'].iloc[-1]
            volume_ok = last_volume > avg_volume * VOLUME_MULTIPLIER

            # ✅ Debug print
            logging.info(f"[VOLUME DEBUG] Last: {last_volume:.2f}, Avg(200): {avg_volume:.2f}, Required: {avg_volume * VOLUME_MULTIPLIER:.2f}, OK: {volume_ok}")
            ...
            logging.info(f"[DEBUG] Prediction: {prediction:.2f}, Volume OK: {volume_ok}, Trend Up: {trend_up}, Close > High: {last_row['close'] > prev_row['high']}")
