...
def run():
    ...
    if open_trade:
        ...
        if exit_info:
            message = (
                f"🏁 EXIT TRADE [{open_trade['direction']}]\n"
                f"Exit Price: {exit_info['exit_price']}\n"
                f"Exit Type: {exit_info['exit_type']}\n"
                f"P/L: {exit_info['pnl']}\n"
                f"Time: {exit_info['exit_time']}"
            )
