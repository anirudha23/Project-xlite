...
VOLUME_MULTIPLIER = float(os.getenv('VOLUME_MULTIPLIER', 1.2))  # ✅ Updated to use .env
...
class StrategyEngine:
    ...
    def detect_trade_signal(self) -> Optional[Dict]:
        candles = self.fetch_candles()
        if len(candles) < 50:
            logging.warning("⛔ Not enough candles for signal.")
            return None
        ...
        try:
            ...
            df = self.calculate_technical_indicators(df)
            if len(df) < 2:
                logging.warning("⛔ Not enough post-indicator data.")
                return None
            ...
            logging.info(f"[DEBUG] Prediction: {prediction:.2f}, Volume OK: {volume_ok}, Trend Up: {trend_up}, Close > High: {last_candle['close'] > prev_candle['high']}")

            if prediction > 0.55 and trend_up and last_candle["close"] > prev_candle["high"] and volume_ok:
            ...
            elif prediction < 0.45 and trend_down and last_candle["close"] < prev_candle["low"] and volume_ok:
                entry = last_candle["close"]
