...
class StrategyEngine:
    ...
    def detect_trade_signal(self) -> Optional[Dict]:
        candles = self.fetch_candles()
        if len(candles) < 70:
            logging.warning("⛔ Not enough candles for signal (need at least 50 for volume average + buffer).")
            return None
        ...
        try:
            ...
            avg_volume = df['volume'].iloc[-51:-1].mean()  # ⬅️ last 50 completed
            last_volume = df['volume'].iloc[-2]  # ⬅️ most recent closed candle
