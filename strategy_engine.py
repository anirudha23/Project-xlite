...
def run():
    ...
    if df is None or df.empty:
        logging.warning("[⚠️] Empty DataFrame received. Skipping this cycle.")
        return
    ...
    if open_trade:
        logging.info("📊 Checking for trade exit...")
        exit_info = check_exit(df, open_trade)
        if exit_info:
            logging.info("✅ Exit condition met. Sending exit signal to Discord.")
            signal = {
                "symbol": SYMBOL,
                "timeframe": INTERVAL,
