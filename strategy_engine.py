...
INTERVAL = os.getenv("TIMEFRAME", "15min")
...
def generate_signal(df):
    ...
    if latest["ai_buy"]:
        ...
        signal_data["sl"] = round(price * 0.99, 2)   # 1% SL
        signal_data["tp"] = round(price * 1.02, 2)   # 2% TP
    elif latest["ai_sell"]:
...
def run_ai_engine():
    print(f"📊 Running strategy for {SYMBOL} on {INTERVAL}")
    df = fetch_candles()
    if df is None:
